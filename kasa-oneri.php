<html>
    <head>
    <meta charset="utf-8">
    <link rel="stylesheet" type="text/css" href="../tasarim/kasa-oneri2.css" />
    ...
    <div class="üst">
        ...
        <body>
               ...
               <?php 
                        ...
                        $sorgu=$baglan->prepare("SELECT baslik,yazi FROM pconeri WHERE kod='KASA'");
                        $sorgu->execute();
                    ...
                    <?php
                        foreach($amd as $bilgi) {?>
                            <h1><?= $bilgi->baslik ?></h1>
                            <img src="../görseller/kasa.png" style="height:250px;">
                            <h3> <?= $bilgi->yazi ?></h3>   
                    <?php }?>
                    <?php 
                        $baglan= new PDO("mysql:host=localhost;dbname=pcmaps;charset=utf8","root","");
                        $sorgu=$baglan->prepare("SELECT yazi FROM pconeri WHERE kod='XPG'");
                        $sorgu->execute();
                        $amd=$sorgu-> fetchAll(PDO::FETCH_OBJ);
                    ?>
                    <?php
                        foreach($amd as $bilgi) {?>
                            <img src="../görseller/xpg-kasa.png" style="height:250px;">
                            <h3> <?= $bilgi->yazi ?></h3>   
                    <?php }?>
                    </div>
               </div>
