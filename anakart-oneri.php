<html>
    <head>
    <meta charset="utf-8">
    <link rel="stylesheet" type="text/css" href="../tasarim/anakart-oneri3.css" />
        ...
        <body>
               ...
               <div class="kutu">
                    <?php 
                        ...
                        $sorgu=$baglan->prepare("SELECT baslik,yazi FROM pconeri WHERE kod='ANAKART'");
                        $sorgu->execute();
                    ...
                    <?php
                        foreach($amd as $bilgi) {?>
                            <h1><?= $bilgi->baslik ?></h1>
                            <img src="../görseller/asus-anakart.png" style="height:250px;">
                            <h3> <?= $bilgi->yazi ?></h3>   
                    <?php }?>
                    <?php 
                        $baglan= new PDO("mysql:host=localhost;dbname=pcmaps;charset=utf8","root","");
                        $sorgu=$baglan->prepare("SELECT yazi FROM pconeri WHERE kod='B550'");
                        $sorgu->execute();
                        $amd=$sorgu-> fetchAll(PDO::FETCH_OBJ);
                    ?>
                    <?php
                        foreach($amd as $bilgi) {?>
                            <img src="../görseller/b450-msi.png" style="height:250px;">
                            <h3> <?= $bilgi->yazi ?></h3>   
                            <img src="../görseller/anakart-karsi.png" >
                    <?php }?>
                    </div>
               </div>
               <div class="alt">
